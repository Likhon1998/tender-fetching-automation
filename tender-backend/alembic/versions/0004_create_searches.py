...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("include_uncategorised", sa.Boolean(), nullable=False, server_default=sa.false()),
        ...
        sa.ForeignKeyConstraint(["created_by"], ["users.id"],
                                name="fk_searches_user", ondelete="SET NULL"),
        sa.CheckConstraint(
            "status IN ('pending', 'running', 'completed', 'failed')",
            name="ck_searches_status",
        ),
    )
    op.create_index("ix_searches_created_at", "searches", [sa.text("created_at DESC")])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["search_id"], ["searches.id"],
                                name="fk_search_tenders_search", ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["tender_id"], ["tenders.id"],
                                name="fk_search_tenders_tender", ondelete="CASCADE"),
    )
